...
class StructDef(Node):
    ...
    def __repr__(self):
        return f"Struct({self.name}<{','.join(self.generics)}>, fields={self.fields})"

class ClassDef(Node):
    def __init__(self, name, fields, methods, generics=None):
        self.name = name
        self.fields = fields  # list of (name, type)
        self.methods = methods  # list of FuncDef
        self.generics = generics or []
    def __repr__(self):
        return f"Class({self.name}<{','.join(self.generics)}>, fields={self.fields}, methods={self.methods})"
"""
Enhanced CPJ parser prototype (line-oriented, indentation-aware for functions)
Supports: print statements, assignments, return, simple expressions (+), def with indented body, GUI blocks.
Emits a small AST and prints it.
"""
import re
import sys
...
class Module(Node):
    def __init__(self, items: List[Node]):
        self.items = items
    def __repr__(self):
        return f"Module({self.items})"


class Print(Node):
    def __init__(self, expr):
        # expr can be a single Expr or a list of Expr
        self.expr = expr
    def __repr__(self):
...
class FuncDef(Node):
    def __init__(self, name, args, body: List[Node]):
        self.name = name
        self.args = args
        self.body = body
    def __repr__(self):
        return f"Func({self.name}, args={self.args}, body={self.body})"
...
class Parser:
    ...
    def parse_def(self):
        header = self.next_line().strip()
        m = re.match(r'def\s+([A-Za-z_][A-Za-z0-9_]*)\s*\(([^)]*)\)\s*:\s*$', header)
        if not m:
...
def parse_expr_text(s: str):
    ...
    def parse_term():
        left = parse_unary()
        while peek() and peek().type in ('STAR', 'SLASH'):
            op = next_tok().value
            right = parse_unary()
